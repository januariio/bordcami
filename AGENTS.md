...
- **Google Maps (optional)**: `VITE_GOOGLE_MAPS_API_KEY` enables city autocomplete, automatic distance calculation (Routes API), and nearby gas station search in the Simulador de Frete. The app uses **REST APIs only** (Places API New, Routes API) — not the legacy JS SDK. The key needs **Places API (New)** and **Routes API** enabled in Google Cloud Console. Without the key, origin/destination fields fall back to plain text inputs and distance must be entered manually.
