...
- **Critical: the `src/` directory is missing from the repository.** The sole commit ("Add files via upload") contains only config files (`vite.config.ts`, `package.json`, `tsconfig*.json`, etc.) but no application source code. As a result, `npm run build` fails (cannot resolve `/src/main.tsx`) and `npm run dev` serves a blank page. The tooling/dependency setup is fully functional — only the source code needs to be added.
